...
nor_population_by_age_original <- function(force_refresh = FALSE) {
  ...
  municip_codes <- meta_07459$Region$values[
    nchar(meta_07459$Region$values) == 4
  ]
  ...
  pop_municip_raw[,
    sex := fcase(
      as.character(Kjonn) == "1" , "male" , as.character(Kjonn) == "2" , "female"
    )
  ]
  ...
  pop_municip_totals <- pop_municip[,
    .(total = sum(population)),
    by = .(municip_code, calyear)
  ]
  pop_municip <- pop_municip[
    pop_municip_totals[total > 0],
    on = .(municip_code, calyear)
  ]
  ...
  pop_ward_raw[,
    municip_code := paste0("municip_nor", substr(ward_numeric, 1, 4))
  ]

  # Assign ward prefixes based on municipality
  pop_ward_raw[, ward_prefix := ""]
  pop_ward_raw[
    municip_code %in% c("municip_nor0301"),
    ward_prefix := "wardoslo_nor"
  ]
  pop_ward_raw[
    municip_code %in% c("municip_nor1201", "municip_nor4601"),
    ward_prefix := "wardbergen_nor"
  ]
  pop_ward_raw[
    municip_code %in% c("municip_nor1103"),
    ward_prefix := "wardstavanger_nor"
  ]
  pop_ward_raw[
    municip_code %in% c("municip_nor1601", "municip_nor5001"),
    ward_prefix := "wardtrondheim_nor"
  ]
  ...
  pop_ward_raw[
    ward_code == "wardoslo_nor030116",
    ward_code := "extrawardoslo_nor030116"
  ]
  pop_ward_raw[
    ward_code == "wardoslo_nor030117",
    ward_code := "extrawardoslo_nor030117"
  ]
  ...
  pop_municip[,
    granularity_geo := stringr::str_extract(location_code, "^[a-z]+")
  ]
