# Validation of the sex dimension added to nor_population_by_age_b2020 / b2024.
...
if(!is.na(baseline) && file.exists(baseline)){
  b <- new.env(); load(baseline, envir = b)
}
...
for(bd in c("b2020", "b2024")){
  new <- copy(get(paste0("nor_population_by_age_", bd), envir = e)); setDT(new)

  # -- schema --
  stopifnot(identical(sort(unique(new$sex)), c("female", "male", "total")))
  stopifnot(identical(sort(names(new)), sort(c("granularity_geo","location_code","age","sex","calyear","pop_jan1_n","imputed"))))

  # -- 1. backward-compat equivalence: total slice identical to baseline --
  if(!is.null(b)){
    old <- copy(get(paste0("nor_population_by_age_", bd), envir = b)); setDT(old)
    nt <- new[sex == "total", c(k, "pop_jan1_n", "imputed"), with = FALSE]
    ot <- old[, c(k, "pop_jan1_n", "imputed"), with = FALSE]  # baseline is total-only
    setkeyv(nt, k); setkeyv(ot, k)
    ...
    cat(sprintf("[%s] total-slice equivalence: %d rows, max|diff| = %g\n", bd, nrow(m), diff_pop))
  } else {
    cat(sprintf("[%s] total-slice equivalence: SKIPPED (no baseline snapshot)\n", bd))
  }

  # -- 3. rectangularity: every cell has exactly male/female/total --
  cnt <- new[, .N, keyby = k]$N
  stopifnot(all(cnt == 3L))
  cat(sprintf("[%s] rectangular: all %d cells have male/female/total\n", bd, length(cnt)))
}
...
new24 <- copy(get("nor_population_by_age_b2024", envir = e)); setDT(new24)
w <- dcast(new24, granularity_geo + location_code + age + calyear + imputed ~ sex, value.var = "pop_jan1_n")
real <- w[!is.na(male)]
...
cat(sprintf("b2024 additivity: nation exact; sub-national max|diff|=%d on %.3f%% of cells (rounding)\n",
            real[, max(abs(diff))], 100 * real[diff != 0, .N] / nrow(real)))
# not-mainland/missing carry NA male/female by design
cat(sprintf("b2024 NA-split cells (Svalbard/JanMayen/unknown): %d\n", nrow(w[is.na(male)])))
# b2020 male/female are all NA (frozen legacy dataset)
new20 <- copy(get("nor_population_by_age_b2020", envir = e)); setDT(new20)
stopifnot(new20[sex != "total", all(is.na(pop_jan1_n))])
cat("b2020 male/female all NA (legacy): OK\n")

# -- 4. source cross-check: national 2026 total = sum over ages, matches SSB fetch --
nat26 <- new24[granularity_geo == "nation" & sex == "total" & calyear == 2026, sum(pop_jan1_n)]
nat_cache <- list.files("data-raw/files/population/api_responses",
                        pattern = "^07459_national_bysex_.*[0-9]\\.rds$", full.names = TRUE)
...
r_sex <- nor_population_by_sex_age_cats(cats = cats, include_total_sex = TRUE, border = 2024)
...
mm <- merge(r_old[, .(granularity_geo,location_code,age,calyear,a=pop_jan1_n)],
...
r_nf <- nor_population_by_sex_age_cats(cats = cats, include_total_sex = FALSE, border = 2024)
stopifnot(identical(sort(unique(r_nf$sex)), c("female", "male")))
r_na <- nor_population_by_sex_age_cats(cats = cats, include_total_age = FALSE, border = 2024)
stopifnot(!"total" %in% unique(r_na$age))
cat("include_total_sex / include_total_age toggles: OK\n")

# 5d. b2020 by_sex runs and returns NA male/female
r20 <- nor_population_by_sex_age_cats(cats = cats, border = 2020)
stopifnot(r20[sex != "total", all(is.na(pop_jan1_n))])
cat("nor_population_by_sex_age_cats(border=2020): OK\n")
